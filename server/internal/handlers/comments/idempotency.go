...
const maxCommentBodyBytes = 1024 * 1024
...
func requireIdempotency(
	...
	return http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		if len(keys) != 1 || httpapi.ValidateIdempotencyKey(keys[0]) != nil {
			writeCommentValidation(response, request, httpapi.FieldError{
		...
		body, err := io.ReadAll(io.LimitReader(request.Body, maxCommentBodyBytes+1))
		if err != nil {
		...
		if !ok {
			writeCommentUnauthenticated(response, request)
			return
		}
