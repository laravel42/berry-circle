...
--
-- The column stays for as long as the OpenFang path does, but it becomes
...
ALTER TABLE model_role_agents
    ALTER COLUMN openfang_agent_id DROP NOT NULL;
...
ALTER TABLE model_role_agents
    DROP CONSTRAINT IF EXISTS model_role_agents_openfang_agent_id_key;
DROP INDEX IF EXISTS model_role_agents_openfang_agent_id_key;

CREATE UNIQUE INDEX IF NOT EXISTS model_role_agents_openfang_agent_id_key
    ON model_role_agents (openfang_agent_id)
    WHERE openfang_agent_id IS NOT NULL;
...
COMMENT ON COLUMN model_role_agents.openfang_agent_id IS
    'The OpenFang agent serving this role, or NULL when the role calls its provider directly.';
