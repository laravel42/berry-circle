...
attachments-by-id, agents, runs, integrations, approvals, the inbox, and the realtime
event streams.

It does **not** yet serve `/api/v1/plans`, `/conversations`, `/search`, `/views`,
`/catalogs`, the multipart upload at `/issues/:ref/attachments`, or `/metrics`.
