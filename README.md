...
attachments-by-id, agents, runs, and the realtime event streams.

It does **not** yet serve `/api/v1/approvals`, `/plans`, `/conversations`, `/integrations`,
`/inbox`, `/search`, `/views`, `/catalogs`, the multipart upload at
