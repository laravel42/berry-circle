...
attachments-by-id, agents, runs, integrations, and the realtime event streams.

It does **not** yet serve `/api/v1/approvals`, `/plans`, `/conversations`, `/inbox`,
`/search`, `/views`, `/catalogs`, the multipart upload at `/issues/:ref/attachments`,
or `/metrics`.
