...
function MetaPill({
   ...
   return (
      <span className="inline-flex items-center gap-1.5 rounded-md border border-border/70 px-2 py-1 text-[11px] text-muted-foreground">
...
function StatusBadge({ status }: { status: string }) {
   ...
   return (
      <span className="inline-flex items-center gap-1.5 rounded-md border border-border/70 px-2 py-1 text-[11px]">
         <span
            className={cn(
...
export default function AgentDetails({ agentId }: AgentDetailsProps) {
   ...
   if (loadError) {
      return (
         <div className="px-8 py-16 text-[11px] text-muted-foreground">
            {loadError}{' '}
   ...
   if (!agent) {
      return <div className="px-8 py-16 text-[11px] text-muted-foreground">Loading agent…</div>;
   }
   ...
   return (
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden text-[11px] leading-[1.35]">
         <div className="border-b px-8 py-6">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
               <div className="flex min-w-0 gap-4">
                  ...
                  <div className="min-w-0">
                     <div className="flex flex-wrap items-center gap-2">
                        <h1 className="text-[11px] font-medium leading-none">{agent.name}</h1>
                     ...
                     {agent.description ? (
                        <p className="mt-2 max-w-3xl text-[11px] leading-[1.35] text-muted-foreground">
                           {agent.description}
                        </p>
                     ) : null}
               ...
               <div className="flex shrink-0 items-center gap-2">
                  <Button size="xs" variant="secondary" className="text-[11px]" disabled>
                     <MessageSquare className="size-4" />
                     DM
                  </Button>
                  <Button size="xs" variant="secondary" className="text-[11px]" asChild>
         ...
         >
               ...
               <TabsList className="h-10 gap-1 rounded-none bg-transparent p-0">
                  {DETAIL_TABS.map((tab) => (
                     <TabsTrigger
                        key={tab}
                        value={tab}
                        className="rounded-none border-b-2 border-transparent px-3 py-2 text-[11px] capitalize data-[state=active]:border-foreground data-[state=active]:bg-transparent data-[state=active]:shadow-none"
                     >
                        {tab}
                     </TabsTrigger>
                  ))}
            ...
            <div className="min-h-0 flex-1 overflow-auto">
                  ...
                  <div className="flex h-full flex-col xl:flex-row">
                     <div className="min-w-0 flex-1 px-8 py-6">
                        <section className="mb-8">
                           <h2 className="text-[11px] font-medium">Now</h2>
                           {activeRuns.length === 0 ? (
                              <div className="mt-3 rounded-lg border border-border/70 px-4 py-5">
                                 <p className="text-[11px] font-medium">No active work</p>
                        ...
                        <section>
                           <h2 className="text-[11px] font-medium">Recent work</h2>
                           {recentRuns.length === 0 ? (
                     ...
                     <aside className="w-full shrink-0 border-t border-border/70 px-8 py-6 xl:w-[320px] xl:border-t-0 xl:border-l">
                        ...
                        <div className="mt-4 rounded-lg border border-border/70 p-4">
                           <h3 className="text-[11px] font-medium">Skills</h3>
                           <p className="mt-2 text-[11px] text-muted-foreground">No skills assigned</p>
                        </div>

                        <div className="mt-4 rounded-lg border border-border/70 p-4">
                           <h3 className="text-[11px] font-medium">Last 30 days</h3>
                           <div className="mt-3 grid grid-cols-2 gap-3 text-[11px]">
                              <div>
                                 <p className="text-[11px] font-medium tabular-nums">{stats.recent.length}</p>
                              ...
                              <div>
                                 <p className="text-[11px] font-medium tabular-nums">{stats.failed.length}</p>
               ...
               <TabsContent value="work" className="mt-0 px-8 py-6">
                  {agentRuns.length === 0 ? (
                     <p className="text-[11px] text-muted-foreground">No runs yet for this agent.</p>
                  ) : (
