...
5. The stream ending after at least one `done` **and** after the runtime's terminal `phase` event (`{"phase":"done"}`) makes the run `succeeded`; ending without that terminal phase — the loop failed mid-way — makes it `failed` with `RUN_INCOMPLETE`, usage kept. The run result is the last substantive turn's text (a closing remark after the answer does not replace the answer): it becomes the run summary and is posted on the issue as the agent's comment. Files the agent writes with `file_write` under `output/` are attached to the run from the stream as well, so a write the runtime drops still reaches the issue.
