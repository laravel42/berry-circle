...
| `settingsNav` in `frontend/components/layout/sidebar/nav-settings.tsx` | I (structure) | B, D, G, H append one item each when their settings page exists |
...
describe(
   ...
   () => {
      ...
      async function insertAgent(workspaceId: string, name: string, archived = false): Promise<string> {
         const [row] = await sql`
            -- `runtime_agent_id` was dropped by migration 032; do not insert it.
