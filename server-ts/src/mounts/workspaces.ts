...
function requireIdempotencyKey(headers: Headers): string {
   // Header.get joins repeats with ", ", so counting entries is the only way
   ...
   const visible = key.length >= 16 && key.length <= 128 && /^[\x21-\x7e]+$/.test(key);
   if (!visible) {
      assertValid([
         fieldError(
 ...
 *
 * Taken over a re-serialisation of the parsed value, not the raw bytes, so
 * whitespace and key order do not make the same request look different — which
 * is what Go's json.Marshal of the decoded value achieves.
 */
function fingerprintJSON(raw: string): Buffer {
...
function serializeMember(member: Membership): Record<string, unknown> {
   return {
