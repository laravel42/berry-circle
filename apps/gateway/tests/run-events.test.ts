...
import { DATABASE_URL, makeTestContext } from "./testkit";
...
describe("GET /api/v1/runs/:runId/events — unauthenticated", () => {
  it("returns 401 UNAUTHENTICATED without a bearer token, even when the run exists", async () => {
...
describeIfDb("GET /api/v1/runs/:runId/events", () => {
  const ctx = makeTestContext();
  let auth: Record<string, string> = {};

  beforeAll(async () => {
    auth = (await ctx.seedSession()).headers;
  });
  ...
  describe("preflight errors", () => {
    ...
    it("returns 400 when both Last-Event-ID and after are supplied", async () => {
      ...
      const res = await app.request(`${path}?after=evt_x`, {
        headers: { ...auth, "Last-Event-ID": "evt_y" },
      });
    ...
    it("returns 400 for a malformed (over-long) cursor", async () => {
      const { app } = setup((s) => s.append(runCreated(0)));
      const res = await app.request(`${path}?after=${"e".repeat(600)}`, { headers: auth });
      expect(res.status).toBe(400);
      const body = (await res.json()) as { error: { code: string } };
      expect(body.error.code).toBe("INVALID_REQUEST");
    });
    ...
    it("returns 409 CURSOR_EXPIRED for an unresolvable cursor", async () => {
      const { app } = setup((s) => s.append(runCreated(0)));
      const res = await app.request(path, {
        headers: { ...auth, "Last-Event-ID": "evt_pruned" },
      });
  ...
  describe("resume", () => {
    it("closes immediately when reconnecting after the terminal event", async () => {
      ...
      const res = await app.request(path, {
        headers: { ...auth, "Last-Event-ID": completed.id },
      });
      const frames = parseSseFrames(await res.text());
    ...
    it("replays only events after a Last-Event-ID cursor", async () => {
      ...
      const res = await app.request(path, {
        headers: { ...auth, "Last-Event-ID": created.id },
      });
      const frames = parseSseFrames(await res.text());
    ...
    it("replays only events after an ?after= cursor", async () => {
      ...
      const res = await app.request(`${path}?after=${created.id}`, { headers: auth });
      const frames = parseSseFrames(await res.text());
      expect(eventNames(frames)).toEqual(["run.output.delta", "run.completed"]);
    });
  });
  ...
  describe("live follow", () => {
    ...
    it("replays the backlog, then follows live until a terminal event closes it", async () => {
      ...
      const res = await app.request(path, { signal: controller.signal, headers: auth });
  ...
  describe("keep-alive & disconnect", () => {
