...
export const boards = pgTable(
  "boards",
  {
    ...
    columns: jsonb("columns").notNull().default(sql`'[]'::jsonb`).$type<BoardColumn[]>(),
    /**
     * Server-side allocator for `issues.number` on this board. Incremented
     * atomically (`UPDATE boards SET issue_counter = issue_counter + 1
     * WHERE id = $1 RETURNING issue_counter`) in the same transaction as the
     * issue insert, so concurrent issue creation never collides.
     */
  ...
  (t) => [
    ...
    check("boards_columns_is_array", sql`jsonb_typeof(${t.columns}) = 'array'`),
  ],
);
...
export const issues = pgTable(
  ...
  (t) => [
    ...
    check(
      "issues_assignee_pair_ck",
      sql`(${t.assigneeType} IS NULL AND ${t.assigneeId} IS NULL) OR (${t.assigneeType} IS NOT NULL AND ${t.assigneeId} IS NOT NULL)`,
    ),
  ],
);
