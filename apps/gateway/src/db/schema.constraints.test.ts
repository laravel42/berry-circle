...
describeIfDb("schema constraints", () => {
  ...
  test("updated_at advances on UPDATE via trigger", async () => {
    ...
    expect(must(updated).updatedAt.getTime()).toBeGreaterThan(must(board).createdAt.getTime());
  });
  ...
  test("users.email uniqueness is case-insensitive", async () => {
    ...
    await expectRejects(db.insert(users).values({ email: email.toLowerCase(), name: "Dup" }));
  });
